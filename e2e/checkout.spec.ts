...
test.describe('Checkout Process', () => {
  ...
  test('should complete full checkout flow for authenticated user', async ({ page }) => {
    ...
    const paymentSection = page.locator('[data-testid="payment-section"], section:has-text("Payment")');
    await expect(paymentSection).toBeVisible();
    
    // Step 8: Fill payment information (test mode)
    const cardNumberInput = page.locator('input[name="cardNumber"], iframe[name*="cardNumber"]');
  ...
  test('should allow editing cart items from checkout', async ({ page }) => {
    ...
    const editCartLink = page.locator('a:has-text("Edit Cart"), button:has-text("Edit"), a[href*="cart"]');
  ...
  test('should handle checkout steps navigation', async ({ page }) => {
    ...
    const steps = page.locator('[data-testid="checkout-steps"], .checkout-steps, .stepper');
    
    if (await steps.isVisible()) {
      // Should show current step highlighted
      const activeStep = page.locator('.active, .current, [aria-current="step"]');
  ...
  test('should handle network errors gracefully', async ({ page }) => {
    ...
    if (await continueButton.isVisible()) {
      ...
      const errorMessage = page.locator('.error, [data-testid="error"], text=/error/i');
