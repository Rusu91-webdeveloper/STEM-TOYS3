...
This document explains how FanCourier is integrated in this project, how the flow works end-to-end, and what you must configure.
...
It uses FanCourier **SelfAWB login** (username/password) to obtain a token. No separate API key is required.
...
Recommended sender defaults (used for display + fallback only; pickup address comes from SelfAWB account):
...
- Shipping method labels: `app/api/checkout/shipping-quote/route.ts` and `features/checkout/components/ShippingMethodSelector.tsx`
...
You can also manually create the AWB from **Admin → Orders → (Order)** using the “Create AWB” button.
...
If disabled, pickup is manual (supplier calls FanCourier or uses their dashboard).
...
FanCourier does **not** receive product details.
The supplier must pack and label the order before pickup.
...
---
If you want multi-supplier split shipments or per-supplier pickup addresses, we can extend the workflow accordingly. 
