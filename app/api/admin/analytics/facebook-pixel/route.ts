...
export const GET = withAdminAuth(async () => {
  try {
    ...
    logger.error("Error fetching Facebook pixel config:", error);
    return NextResponse.json(
      { error: "Failed to fetch Facebook pixel configuration" },
      { status: 500 }
    );
  }
});
...
export const PUT = withAdminAuth(async (request: NextRequest) => {
  try {
    ...
    const existingConfig = await db.facebookPixelConfig.findFirst();

    if (existingConfig) {
      // Update existing config
      const updatedConfig = await db.facebookPixelConfig.update({
      ...
      logger.info("Facebook pixel config updated", {
      ...
      // Create new config
      const newConfig = await db.facebookPixelConfig.create({
        data: {
          pixelId,
          accessToken,
      ...
      logger.info("Facebook pixel config created", { configId: newConfig.id });
      return NextResponse.json({
        id: newConfig.id,
    ...
    logger.error("Error updating Facebook pixel config:", error);
    return NextResponse.json(
      { error: "Failed to update Facebook pixel configuration" },
      { status: 500 }
    );
  }
});
