  ...
  try {
    ...
    if (!config) {
      return NextResponse.json(
        {
          success: false,
          error: "No active Instagram pixel configuration found",
        },
        { status: 404 }
      );
    }

    // Test basic pixel ID format (Instagram uses Facebook pixel format)
    if (
      !config.pixelId ||
    ...
    if (config.accessToken) {
      try {
        ...
        logger.error("Instagram API test error:", apiError);
        apiTestResult = {
          success: false,
          message: "Failed to connect to Instagram Conversions API",
        };
      }
    }

    logger.info("Instagram pixel test completed", {
    ...
    return NextResponse.json({
      ...
      message: "Instagram pixel configuration is valid",
    });
  } catch (error) {
    logger.error("Error testing Instagram pixel:", error);
    return NextResponse.json(
      { success: false, error: "Failed to test Instagram pixel configuration" },
      { status: 500 }
    );
  }
});
