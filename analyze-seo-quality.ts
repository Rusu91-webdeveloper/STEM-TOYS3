...
async function analyzeSEOQuality() {
  try {
    const productId = "cmgjykce7000b1kzx1o8al1rf"; // K'NEX Education Set - AI Enhanced
    ...
    console.log(
      `Quality: ${descWordCount >= 300 ? "✅ GOOD" : descWordCount >= 150 ? "⚠️  OK" : "❌ TOO SHORT"}`
    );
    ...
    console.log(
      `Quality: ${product.tags.length >= 15 ? "✅ EXCELLENT" : product.tags.length >= 10 ? "⚠️  GOOD" : "❌ NEEDS MORE"}`
    );
    ...
    if (product.metadata) {
      ...
      if (meta.seo) {
        ...
        console.log(
          `  Quality: ${titleLength >= 50 && titleLength <= 60 ? "✅ PERFECT" : titleLength >= 40 ? "⚠️  OK" : "❌ TOO SHORT"}`
        );
        ...
        console.log(
          `  Quality: ${descLength >= 150 && descLength <= 160 ? "✅ PERFECT" : descLength >= 120 ? "⚠️  OK" : "❌ TOO SHORT"}`
        );
        ...
        console.log(
          `  Quality: ${keywordsCount >= 20 ? "✅ EXCELLENT" : keywordsCount >= 10 ? "⚠️  GOOD" : "❌ NEEDS MORE"}`
        );
        console.log(
          `  Keywords: ${meta.seo.metaKeywords?.slice(0, 10).join(", ")}...`
        );
      } else {
      ...
      console.log(
        `Learning Outcomes: ${meta.learningOutcomes?.length || 0} items → ${meta.learningOutcomes?.join(", ") || "None"}`
      );
      console.log(
        `Romanian Competencies: ${meta.romanianCompetencies?.length || 0} items`
      );
      if (meta.romanianCompetencies?.length > 0) {
        meta.romanianCompetencies
          .slice(0, 3)
          .forEach((comp: string, i: number) => {
      ...
      if (meta.romanianCurriculumAlignment?.length > 0) {
        meta.romanianCurriculumAlignment
          .slice(0, 3)
          .forEach((curr: string, i: number) => {
            console.log(`  ${i + 1}. ${curr}`);
          });
      }
      console.log(
        `Subject Areas: ${meta.romanianSubjectAreas?.length || 0} items → ${meta.romanianSubjectAreas?.join(", ") || "None"}`
      );
      console.log(
    ...
    if (product.attributes) {
      ...
      if (attrs.specs) {
        ...
        console.log(
          `Quality: ${specsCount >= 10 ? "✅ EXCELLENT" : specsCount >= 5 ? "⚠️  GOOD" : "❌ NEEDS MORE"}`
        );
        console.log(`Specs:`);
        Object.entries(attrs.specs)
          .slice(0, 10)
          .forEach(([key, value]) => {
    ...
    console.log(
      `Grade: ${score >= 90 ? "🏆 A+" : score >= 80 ? "✅ A" : score >= 70 ? "⚠️  B" : score >= 60 ? "⚠️  C" : "❌ D"}`
    );
    console.log(
    ...
    if (descWordCount < 300)
      recommendations.push("❌ Increase description to 400-600 words");
    if (product.tags.length < 15)
      recommendations.push("❌ Add more tags (target: 15-20 bilingual)");
    if (!meta?.seo?.metaTitle || meta.seo.metaTitle.length < 50)
      recommendations.push("❌ Improve meta title (50-60 chars)");
    if (!meta?.seo?.metaDescription || meta.seo.metaDescription.length < 150)
      recommendations.push("❌ Improve meta description (150-160 chars)");
    if (!meta?.seo?.metaKeywords || meta.seo.metaKeywords.length < 20)
      recommendations.push("❌ Add more keywords (target: 25-35)");
    if (!meta?.learningOutcomes || meta.learningOutcomes.length < 3)
      recommendations.push("❌ Add learning outcomes (target: 3-5)");
    if (!meta?.romanianCompetencies || meta.romanianCompetencies.length < 3)
      recommendations.push("❌ Add Romanian competencies (target: 3-5)");
    if (!attrs?.specs || Object.keys(attrs.specs).length < 10)
      recommendations.push("❌ Add more product specs (target: 12-15)");
