...
export function ProductCard({
  ...
  const productImages =
    Array.isArray(product.images) && product.images.length > 0
      ...
      : [];
  const imageUrl =
    productImages[activeImageIndex] || "/placeholder-product.png";
  ...
  return (
    <div
      className={cn(
        "group relative flex h-full flex-col overflow-hidden rounded-[1.25rem] border border-slate-200/80 bg-white shadow-[0_18px_48px_-40px_rgba(15,23,42,0.35)] transition-all duration-300 hover:-translate-y-1 hover:border-blue-200 hover:shadow-[0_28px_58px_-38px_rgba(37,99,235,0.24)]",
        isBundle && "border-cyan-200",
        className
      )}
    >
      ...
      <div className="relative aspect-square overflow-hidden bg-[linear-gradient(145deg,#f8fafc,#eef2f7)]">
        ...
        <button
          ...
          className={cn(
            "absolute right-2.5 top-2.5 z-20 flex h-9 w-9 items-center justify-center rounded-full border shadow-sm backdrop-blur transition-all duration-200",
            "opacity-100",
            isFavorited
              ? "border-rose-200 bg-rose-50 text-rose-600"
              : "border-slate-100 bg-white/90 text-slate-400 hover:border-rose-100 hover:text-rose-500",
              ...
              className={cn(
                ...
                justAdded
                  ...
                  : isBundle
                    ...
                    : "bg-[#0b1220] text-white hover:bg-blue-600"
              )}
            >
              {isAddingToCart ? (
