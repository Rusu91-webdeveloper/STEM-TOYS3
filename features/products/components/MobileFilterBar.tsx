...
export function MobileFilterBar({
  ...
  return (
    <div className="md:hidden sticky top-[88px] sm:top-[96px] z-30 bg-gradient-to-b from-white via-white/95 to-white/90 backdrop-blur-lg border-b border-gray-100/60 shadow-lg animate-in slide-in-from-top duration-700">
      ...
      <div className="px-4 py-2.5 border-b border-gray-50/80">
        <div className="flex items-center justify-between">
          ...
          {activeFilterCount > 0 && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-gray-600 hover:text-gray-800 hover:bg-gray-100 rounded-full text-xs font-medium transition-all duration-300 hover:scale-105 active:scale-95"
            >
      ...
      {/* Main Filter Content - Single Row Layout */}
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <button
              ...
              className="flex-shrink-0 flex items-center gap-1 px-2 py-1.5 bg-gradient-to-r from-red-100 to-orange-100 text-red-700 border border-red-200 rounded-full text-xs font-medium hover:from-red-200 hover:to-orange-200 transition-all duration-300 transform hover:scale-105 active:scale-95"
            >
              <Star className="w-3 h-3" />
              <span className="hidden sm:inline">{t("best", "Top")}</span>
            </button>
            <button
              onClick={() => onCategoryQuickSelect("NEW_ARRIVALS")}
              className="flex-shrink-0 flex items-center gap-1 px-2 py-1.5 bg-gradient-to-r from-yellow-100 to-amber-100 text-yellow-700 border border-yellow-200 rounded-full text-xs font-medium hover:from-yellow-200 hover:to-amber-200 transition-all duration-300 transform hover:scale-105 active:scale-95"
            >
              <Zap className="w-3 h-3" />
              <span className="hidden sm:inline">{t("new", "Nou")}</span>
            </button>
            <button
              onClick={() => onCategoryQuickSelect("GIFT_IDEAS")}
              className="flex-shrink-0 flex items-center gap-1 px-2 py-1.5 bg-gradient-to-r from-purple-100 to-pink-100 text-purple-700 border border-purple-200 rounded-full text-xs font-medium hover:from-purple-200 hover:to-pink-200 transition-all duration-300 transform hover:scale-105 active:scale-95"
            >
              <Gift className="w-3 h-3" />
              <span className="hidden sm:inline">{t("gifts", "Cadouri")}</span>
            </button>
          </div>
