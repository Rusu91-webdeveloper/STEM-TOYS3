  ...
  return (
    <section className="px-4 sm:px-6 lg:px-8">
      <div className="container mx-auto max-w-7xl">
        <div className="relative overflow-hidden rounded-[2.25rem] border border-slate-200/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.92)_0%,rgba(248,252,255,0.96)_100%)] px-6 py-8 shadow-[0_35px_90px_-55px_rgba(15,23,42,0.45)] sm:px-8 sm:py-10 lg:px-10">
          <div
            aria-hidden
          ...
          <div className="relative z-10">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-[56rem]">
                ...
                <h2 className="mt-4 max-w-5xl text-[2rem] font-black leading-[0.98] tracking-[-0.04em] text-slate-950 sm:text-[2.6rem] lg:text-[3.35rem]">
                  Incepe din colectia potrivita, nu dintr-un catalog prea mare
                </h2>
                <p className="mt-4 max-w-xl text-sm leading-7 text-slate-600 sm:text-base lg:text-[1.05rem]">
                  Alege ruta potrivita si ajungi mai repede la produsele relevante.
                </p>
              </div>
              <Link
                href="/products"
                className="inline-flex h-12 items-center justify-center rounded-2xl border border-slate-200/90 bg-white/95 px-5 text-sm font-bold text-slate-800 shadow-[0_18px_40px_-32px_rgba(15,23,42,0.35)] transition hover:border-slate-300 hover:text-slate-950"
              >
                Vezi tot catalogul
              </Link>
            </div>
              ...
              {nationalCommercialRoutes.map((journey, index) => {
                ...
                return (
                  <Link
                    key={journey.href}
                    href={journey.href}
                    className={`group relative overflow-hidden rounded-[1.7rem] border p-5 transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_26px_55px_-42px_rgba(15,23,42,0.28)] ${isFeatured ? `${accent.shell} border-white/80 shadow-[0_30px_70px_-48px_rgba(15,23,42,0.35)] md:col-span-2 xl:col-span-2 xl:grid xl:grid-cols-[minmax(0,1.12fr)_minmax(250px,0.72fr)] xl:gap-6 xl:p-6` : "border-slate-200/90 bg-white/88 backdrop-blur-sm hover:border-slate-300/90"}`}
                  >
                    <div className={isFeatured ? "xl:flex xl:flex-col xl:justify-between" : ""}>
                      ...
                      <div className="mt-5">
                        <div className="flex items-center gap-3">
                          <span className={`inline-flex h-12 w-12 items-center justify-center rounded-2xl text-slate-900 ${isFeatured ? "border border-white/80 bg-white/85 shadow-[0_16px_30px_-24px_rgba(15,23,42,0.28)]" : "border border-slate-200/80 bg-slate-50/90"}`}>
                            <Icon className="h-5 w-5" />
                          </span>
                          <div>
                            <p className="text-[1.8rem] font-black tracking-[-0.03em] text-slate-950">
                              {journey.label}
                            </p>
                            <p className="mt-1 text-[13px] font-medium uppercase tracking-[0.14em] text-slate-400">
                              {meta?.hint}
                            </p>
                          </div>
                        </div>
                        <p className="mt-4 max-w-[36rem] text-sm leading-6 text-slate-600">
                          {journey.description}
                        </p>
                      </div>
                      ...
                      <div className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-slate-800">
                        <span className={isFeatured ? accent.link : "text-slate-800 group-hover:text-slate-950"}>
                    ...
                    {isFeatured ? (
                      <div className="mt-5 rounded-[1.5rem] border border-white/80 bg-white/78 p-5 shadow-[0_20px_40px_-32px_rgba(15,23,42,0.22)] backdrop-blur-sm xl:mt-0 xl:p-6">
                        <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-slate-400">
                          Ruta principala
                        </p>
                        <h3 className="mt-3 text-[1.65rem] font-black leading-[1.02] tracking-[-0.03em] text-slate-950">
                          Intrarea rapida pentru cautarile comerciale mari
                        </h3>
                        <div className="mt-4 grid gap-2.5">
                          <div className="rounded-2xl border border-slate-200/80 bg-white/92 px-3 py-2.5 text-sm font-medium text-slate-700">
            ...
            <div className="mt-11 border-t border-slate-200/80 pt-7">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div className="max-w-2xl">
                  ...
                  <h3 className="mt-3 text-[1.9rem] font-black leading-[1.02] tracking-[-0.03em] text-slate-950">
                    Orasele mari raman la un click distanta
                  </h3>
                  <p className="mt-2 text-sm leading-6 text-slate-600">
                    Paginile locale raman accesibile, intr-un format mai discret.
                  </p>
                </div>
                <Link
                  href="/jucarii-stem"
                  className="inline-flex h-11 items-center justify-center rounded-2xl border border-slate-200/90 bg-white px-4 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:text-slate-950"
                >
                  Vezi hubul national STEM
                </Link>
              </div>

              <div className="mt-5 rounded-[1.65rem] border border-slate-200/80 bg-white/82 p-4 shadow-[0_20px_45px_-40px_rgba(15,23,42,0.24)] backdrop-blur-sm sm:p-5">
                <div className="flex flex-wrap items-center gap-2.5">
                  {regionalLinks.map(link => (
                    <Link
                      key={`${link.href}-chip`}
                      href={link.href}
                      className="inline-flex items-center gap-2 rounded-full border border-slate-200/90 bg-slate-50/80 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-white hover:text-slate-950"
                    >
                ...
                <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                  {regionalLinks.slice(0, 3).map(link => (
                    <Link
                      key={link.href}
                      href={link.href}
                      className="group rounded-[1.35rem] border border-slate-200/80 bg-white/94 px-4 py-4 transition duration-300 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-[0_18px_38px_-34px_rgba(15,23,42,0.2)]"
                    >
                      ...
                      <p className="mt-4 text-[1.45rem] font-black leading-[1.05] tracking-[-0.03em] text-slate-950">
                        {link.label}
                      </p>
                      <p className="mt-2 text-sm leading-6 text-slate-600">
                        STEM, educative si robotica pentru cautari locale cu intentie clara.
                      </p>
                ...
                <div className="mt-5 flex flex-col gap-2 rounded-[1.25rem] border border-dashed border-slate-200 bg-slate-50/70 px-4 py-4 text-sm text-slate-600 sm:flex-row sm:items-center sm:justify-between">
                  <span>
                    Bucuresti, Cluj si Timisoara raman intrarile regionale principale.
                  </span>
