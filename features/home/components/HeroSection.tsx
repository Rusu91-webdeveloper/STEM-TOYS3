...
import { HoverRacerPreview } from "./HoverRacerPreview";
...
export const HeroSection = React.memo(({
  ...
  const trust = [
    ...
    { icon: Sparkles, label: "Branduri europene" },
    { icon: CreditCard, label: "Netopia / Stripe" },
  ];
  return (
    <section aria-labelledby="home-title" className="bg-[#0b1b32] text-white">
      <div className="mx-auto grid max-w-7xl items-center gap-6 px-5 py-8 sm:px-8 lg:grid-cols-[1.4fr_1fr] lg:gap-12 lg:py-10">
        <div>
          ...
          <p className="mt-4 max-w-xl text-base leading-7 text-slate-200">
            Jucării de construit, experimente și descoperiri pe bune. Alege după
            vârstă și transformă curiozitatea în „uite ce am făcut!”.
          </p>
        ...
        <div className="mx-auto w-full max-w-[280px] sm:max-w-[380px]">
          <HoverRacerPreview />
          <Link
            href={`/products/${HOVER_RACER_SLUG}`}
            className="mt-3 flex min-h-12 items-center justify-between gap-3 rounded-lg focus-visible:outline focus-visible:outline-2 focus-visible:outline-emerald-200"
          >
            <span>
              <span className="block text-[10px] font-semibold uppercase tracking-widest text-emerald-200">
                Recomandarea săptămânii · 8+ ani
              </span>
          ...
          <p className="mt-1 text-xs text-slate-300">
            Construiește un aeroglisor. Descoperă forța aerului.
          </p>
        </div>
      </div>
    </section>
