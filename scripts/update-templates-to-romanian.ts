...
async function updateTemplatesToRomanian() {
  console.log("🔄 Se actualizează șabloanele de email în limba română...\n");

  const romanianTemplates = [
    {
  ...
  for (const templateData of romanianTemplates) {
    try {
      ...
      console.log(`✅ Actualizat șablonul: ${result.name}`);
      updatedCount++;
    } catch (error) {
      console.error(
        `❌ Eroare la actualizarea șablonului ${templateData.slug}:`,
        error
      );
    }
  }

  console.log(`\n🎉 Actualizare șabloane completată!`);
  console.log(`   • Actualizate: ${updatedCount} șabloane în limba română`);
  console.log(`   • Sistemul de email este acum complet în limba română! 🇷🇴`);
}

updateTemplatesToRomanian()
  .catch(console.error)
