/**
 * Dual Provider Blog Enhancement Service Tests
 * Tests for the AI blog generation service
 */
...
describe("OpenAIBlogEnhancementService", () => {
  ...
  describe("Initialization", () => {
    it("should initialize with default config", async () => {
      ...
      // Trigger service initialization by calling a method that uses initServices
      const mockPrompt = { prompt: "test" };
      try {
        await service.generateBlog(mockPrompt, {});
      } catch (error) {
        // Expected to fail due to mocking, but should have called initServices
      }

      expect(mockAIServiceFactory.getService).toHaveBeenCalledWith("gemini");
    ...
    it("should initialize with custom config", () => {
      const customConfig = {
        ...
        secondaryProvider: "anthropic" as const,
      };

      // Need to mock the service calls for custom config too
      mockAIServiceFactory.getService
        .mockImplementationOnce(() => mockPrimaryService)
        .mockImplementationOnce(() => mockSecondaryService);

      service = new DualProviderBlogEnhancementService(customConfig);

      expect(service).toBeDefined();
      // Custom config should override defaults
      expect(mockAIConfig.getProvider).toHaveBeenCalled();
      expect(mockAIConfig.getModel).toHaveBeenCalled();
    });
  });
  ...
  describe("Blog Generation", () => {
    ...
    beforeEach(() => {
      ...
      mockPrimaryService.generateResponse.mockResolvedValueOnce(
...
Descoperiți colecția noastră completă de jucării STEM!
      ...
      mockSecondaryService.generateResponse
...
Long-tail Keywords: jucării STEM pentru copii, beneficii jucării educaționale, cum aleg jucării STEM, dezvoltarea cognitivă copii`);
      ...
      mockSecondaryService.generateResponse.mockResolvedValueOnce(`
# Jucăriile STEM în 2025: Revoluția Educației Digitale
...
Descoperiți colecția noastră completă de jucării STEM!
  ...
  describe("Progress Tracking", () => {
    it("should call progress callback during generation", async () => {
      ...
      mockPrimaryService.generateResponse.mockResolvedValue("Test Title");
      mockPrimaryService.generateResponse.mockResolvedValue("Test content");
      mockSecondaryService.generateResponse.mockResolvedValue(
  ...
  describe("Error Handling", () => {
    it("should handle AI service failures gracefully", async () => {
      mockPrimaryService.generateResponse.mockRejectedValue(
  ...
  describe("SEO Analysis", () => {
    it("should calculate SEO score", async () => {
      mockPrimaryService.generateResponse
