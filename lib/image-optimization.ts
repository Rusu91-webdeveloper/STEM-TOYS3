...
export class ImageOptimizer {
  ...
  generateSrcSet(baseSrc: string, sizes: number[] = this.config.sizes): string {
    return sizes
      .map(size => `${this.optimizeImageUrl(baseSrc, { width: size })} ${size}w`)
      .join(', ');
  }
   ...
   */
  getOptimalDimensions(containerWidth: number, aspectRatio: number = 1): {
    width: number;
    height: number;
  } {
   ...
   */
  shouldPrioritize(imageContext: 'hero' | 'above-fold' | 'below-fold' | 'lazy'): boolean {
  ...
  isValidImageUrl(url: string): boolean {
    try {
      ...
      return this.config.remotePatterns.some(pattern => {
        ...
        const pathnameMatch = !pattern.pathname || urlObj.pathname.startsWith(pattern.pathname);
   ...
   */
  getImageStats(images: Array<{ src: string; width?: number; height?: number; }>): {
    totalImages: number;
    averageSize: number;
    ...
    const modernFormatRatio = (stats.formats['webp'] || 0) + (stats.formats['avif'] || 0);
...
export function getOptimizedImageProps(options: Parameters<ImageOptimizer['getImageProps']>[0]) {
