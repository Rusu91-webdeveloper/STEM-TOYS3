...
export class JWTValidator {
  ...
  async validateToken(
    ...
    try {
      ...
      if (options.requireClaims) {
        const missingClaims = this.validateRequiredClaims(
          payload,
  ...
  private async getCachedResult(
    token: string
  ): Promise<Omit<ValidationResult, "responseTime"> | null> {
    try {
      const cacheKey = `jwt_validation:${token.slice(-8)}`;
  ...
  private updateStats(responseTime: number): void {
    ...
    this.stats.averageTime =
      (this.stats.averageTime * (totalValidations - 1) + responseTime) /
      totalValidations;
  }
...
export async function validateJWT(
  token: string,
  options?: Parameters<JWTValidator["validateToken"]>[1]
) {
