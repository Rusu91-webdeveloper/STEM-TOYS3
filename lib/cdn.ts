...
const defaultConfig: CDNConfig = {
  enabled:
    process.env.CDN_ENABLED === "true" || process.env.NODE_ENV === "production",
...
export class CDNManager {
  ...
  getAssetUrl(
    ...
    try {
      ...
      const domain =
        this.config.domains[
          assetConfig.domain as keyof typeof this.config.domains
        ];
      if (!domain) {
      ...
      if (
        assetConfig.optimization &&
        this.config.optimization.enableImageResize
      ) {
        ...
        if (options.quality) params.set("q", options.quality.toString());
          ...
          params.set("q", this.config.optimization.defaultQuality.toString());
      ...
      logger.debug("CDN URL generated", {
        originalPath: path,
        cdnUrl: url,
        assetType,
      });
  ...
  getPictureElement(
    ...
    if (this.config.optimization.enableAVIF) {
      const avifSrcSet = this.getResponsiveImageSrcSet(path, widths, {
        quality,
        format: "avif",
      });
    ...
    if (this.config.optimization.enableWebP) {
      const webpSrcSet = this.getResponsiveImageSrcSet(path, widths, {
        quality,
        format: "webp",
      });
    ...
    // Fallback JPEG/PNG
    const fallbackSrcSet = this.getResponsiveImageSrcSet(path, widths, {
      quality,
      format: "auto",
    });
    const fallbackSrc = this.getAssetUrl(path, {
      type: "image",
  ...
  private signUrl(url: string): string {
    ...
    try {
      ...
      const expiry =
        Math.floor(Date.now() / 1000) + this.config.security.tokenExpiry;
      const token = Buffer.from(`${url}:${expiry}`)
        .toString("base64")
        .substring(0, 16);
...
export function getImageUrl(
  path: string,
  options?: Parameters<CDNManager["getAssetUrl"]>[1]
): string {
...
export function generatePictureElement(
  path: string,
  alt: string,
  options?: Parameters<CDNManager["getPictureElement"]>[2]
): string {
