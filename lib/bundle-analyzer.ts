...
export class BundleSplitter {
  ...
  createLazyComponent<T extends ComponentType<any>>(
    ...
    const WrappedComponent = error
      ? this.withErrorBoundary(LazyComponent, error, name)
      : LazyComponent;
  ...
  createRoute(
    ...
    return this.createLazyComponent(importFn, {
      ...
      preload:
        options.preload ||
        this.config.preload.critical.includes(`/app/${name}/page`),
    });
  }
  ...
  createFeature(
    ...
    const isImportant = this.config.preload.important.includes(
      `/features/${featureName}/components/${componentName}`
    );
...
export class WebpackBundleAnalyzer {
  ...
  async analyzeBundles(statsJson?: any): Promise<BundleStats | null> {
    try {
      ...
      const mockStats: BundleStats = {
        ...
        assets: 45,
        modules: 230,
        duplicateModules: ["lodash", "react-dom/client", "next/dynamic"],
        largeAssets: [
  ...
  getRecommendations(): string[] {
    ...
    if (this.stats.totalSize > 3 * 1024 * 1024) {
      ...
      recommendations.push(
        "Total bundle size is large (>3MB). Consider more aggressive code splitting."
      );
    }
    ...
    if (largeJSAssets.length > 0) {
      recommendations.push(
        `Large JavaScript assets detected: ${largeJSAssets.map(a => a.name).join(", ")}`
      );
    }
    ...
    if (this.stats.duplicateModules.length > 0) {
      recommendations.push(
        `Duplicate modules found: ${this.stats.duplicateModules.join(", ")}`
      );
    }
    ...
    if (compressionRatio > 0.4) {
      recommendations.push(
        "Poor compression ratio. Consider optimizing assets and removing unused code."
      );
    }
  ...
  private logAnalysis(stats: BundleStats): void {
    const compressionRatio = (
      (1 - stats.gzippedSize / stats.totalSize) *
      100
    ).toFixed(1);
...
// Note: createIntersectionLazyComponent has been moved to bundle-analyzer-client.ts
// for client-side usage only, as it requires React hooks
