...
export function generateProductInternalLinks(product: Product): InternalLink[] {
  ...
  if (product.stemDiscipline) {
    links.push({
      url: `/stem-discipline/${product.stemDiscipline.toLowerCase()}`,
  ...
  links.push({
    ...
    context: "Citește ghidul nostru complet pentru a înțelege cum să integrezi educația STEM în dezvoltarea copilului",
    relevanceScore: 0.6,
    linkType: "educational"
  });
  ...
  if (product.learningOutcomes && product.learningOutcomes.length > 0) {
    ...
    links.push({
      url: `/dezvoltare-abilitati/${primaryOutcome.toLowerCase().replace('_', '-')}`,
  ...
  if (product.romanianMinistryApproval) {
    links.push({
      url: "/certificare-mects-jucarii-stem",
      anchor: "certificarea MECTS pentru jucării educaționale",
      context: "Află de ce certificarea MECTS este importantă pentru siguranța și calitatea educațională",
      relevanceScore: 0.5,
      linkType: "educational"
    });
  }
 ...
 */
export function generateRelatedProductLinks(product: Product, allProducts: Product[]): InternalLink[] {
  const links: InternalLink[] = [];
  ...
  const sameDisciplineProducts = allProducts
    .filter(p => p.id !== product.id && p.stemDiscipline === product.stemDiscipline)
    .slice(0, 2);
 ...
 */
export function generateTopicClusterLinks(currentTopic: string): InternalLink[] {
  const topicClusters = {
    "educatie-stem": [
      {
  ...
  return topicClusters[currentTopic as keyof typeof topicClusters]?.map(link => ({
    ...link,
 ...
 */
export function generateAnchorTextVariations(baseAnchor: string, product?: Product): string[] {
...
function getAgeGroupLandingPageUrl(ageGroup: string): string {
  const urlMap = {
    "TODDLERS_1_3": "/jucarii-stem-copii-1-3-ani",
    "PRESCHOOL_3_5": "/jucarii-stem-copii-3-5-ani",
    "ELEMENTARY_6_8": "/jucarii-stem-copii-6-8-ani", 
    "MIDDLE_SCHOOL_9_12": "/jucarii-stem-copii-9-12-ani",
    "TEENS_13_PLUS": "/jucarii-stem-adolescenti-13-plus",
  };
...
function getLearningOutcomeDisplayName(outcome: string): string {
  ...
  return outcomeMap[outcome as keyof typeof outcomeMap] || outcome.toLowerCase();
}
 ...
 */
export function renderInternalLink(link: InternalLink, className?: string): string {
