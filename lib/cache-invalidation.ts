...
export class CacheInvalidationManager {
  ...
  async invalidateEntity(
    ...
    try {
      ...
      logger.info(`Cache invalidated for ${entityType}:${id}`, {
        reason,
        entityType,
        id,
      });
      ...
      logger.error(`Failed to invalidate cache for ${entityType}:${id}`, {
        error,
        entityType,
        id,
      });
  ...
  private hasListAffectingFields(
    entityType: string,
    fields: string[]
  ): boolean {
...
export async function invalidateCart(
  userId: string,
  reason?: string
): Promise<void> {
...
export async function invalidateBook(
  id: string,
  reason?: string
): Promise<void> {
